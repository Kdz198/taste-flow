...
export default function CartContent() {
        ...
        const fetchCartItems = async () => {
            try {
                const itemsWithDetails = itemsToAdd.map((cartItem) => {
                    const product = menuList?.find((p: FoodType) => p._id === cartItem.itemId);
                ...
                if (itemsWithDetails.length > 0) {
                    const allItemIds = new Set(itemsWithDetails.map(item => item._id));
    ...
    const deliveryFee = selectedCartItems.length > 0 ? 15000 : 0; // 15,000 VND delivery fee
                ...
                <div className="bg-gradient-to-br from-[#2A2A2A] to-[#1A1A1A] rounded-2xl p-6 border border-[#3A3A3A]">
                    ...
                    <div className="space-y-4">
                        {cartItems.map((item) => (
                            <div 
                                key={item._id}
                                className={`flex items-center gap-4 p-4 bg-[#1A1A1A] rounded-xl border transition-all duration-300 ${
                            ...
                            >
                                ...
                                <Checkbox
                                    ...
                                    onCheckedChange={(checked) => handleItemSelect(item._id, checked as boolean)}
                                    className="data-[state=checked]:bg-[#F26D16] data-[state=checked]:border-[#F26D16]"
                                />
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-[#858787] text-sm">
                                        {item.receipt?.length || 0} ingredients
                                    </p>
                                </div>
            ...
            <div className="lg:col-span-1">
                <div className="bg-gradient-to-br from-[#2A2A2A] to-[#1A1A1A] rounded-2xl p-6 border border-[#3A3A3A] sticky top-8">
                    ...
                    {selectedItems.size > 0 ? (
                        ...
                        >
                            <Button className="w-full bg-[#F26D16] hover:bg-orange-600 text-white rounded-xl py-5 font-semibold text-lg transition-all duration-300 hover:scale-[1.02]">
                                Proceed to Payment 
                            </Button>
                        </Link>
                    ) : (
